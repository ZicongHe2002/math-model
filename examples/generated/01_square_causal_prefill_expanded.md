...
| N_Kproj[executed_graph] | `work.projection.rows.N_Kproj.executed` | `B*H*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 1280 | rows | partial | scheduled_lengths | K rows projected in this invocation as the executed graph performs them, including the KV-loop multiplicity and excluding tiles no query block scans (spec 5.2) | N_Kproj, the once-per-row count of the declared scope |
...
| N_Vproj[executed_graph] | `work.projection.rows.N_Vproj.executed` | `B*H*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 1280 | rows | partial | scheduled_lengths | V rows projected in this invocation as the executed graph performs them, including the KV-loop multiplicity and excluding tiles no query block scans (spec 5.2) | N_Vproj, the once-per-row count of the declared scope |
...
| W_vec[mul] | `work.vector.mul` | `B*D_v*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 52,480 | element-ops | partial | scheduled_lengths | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[mask] | `work.vector.mask` | `B*H*BlockStat(masked_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 8192 | element-ops | partial | scheduled_lengths | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[cmp] | `work.vector.cmp` | `B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 20,480 | element-ops | partial | scheduled_lengths | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[broadcast] | `work.vector.broadcast` | `B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 20,480 | element-ops | partial | broadcast_materialized (undeclared backend behaviour), scheduled_lengths | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[transpose] | `work.vector.transpose` | `B*D_n*H*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 30,720 | element-ops | partial | operand_transpose_materialized (undeclared backend behaviour), scheduled_lengths | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[exp] | `work.vector.exp` | `B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 21,760 | element-ops | partial | scheduled_lengths | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[add] | `work.vector.add` | `B*D_v*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*H*S_q + B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + Piecewise((B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k), D_r > 0), (0, True))` | 72,192 | element-ops | partial | scheduled_lengths | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[div] | `work.vector.div` | `B*D_v*H*S_q` | 12,288 | element-ops | partial | final_normalize, scheduled_lengths | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[recip] | `work.vector.recip` | `0` | 0 | element-ops | partial | scheduled_lengths | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[log] | `work.vector.log` | `B*H*S_q` | 512 | element-ops | partial | scheduled_lengths | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[cast] | `work.vector.cast` | `B*D_v*H*S_q + B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 32,768 | element-ops | partial | cast_points, scheduled_lengths | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_resource[vpu] | `work.resource.vpu` | `2*B*D_v*H*S_q + 2*B*D_v*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*H*S_q + B*H*BlockStat(masked_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + Piecewise((B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k), D_r > 0), (0, True))` | 159,232 | element-ops | partial | cast_points, final_normalize, scheduled_lengths | per resource class u (for W_u/P_u) |  |
| W_resource[reduce] | `work.resource.reduce` | `2*B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) - B*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 39,680 | element-ops | partial | scheduled_lengths | per resource class u (for W_u/P_u) |  |
| W_resource[layout] | `work.resource.layout` | `B*D_n*H*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 51,200 | element-ops | partial | broadcast_materialized (undeclared backend behaviour), operand_transpose_materialized (undeclared backend behaviour), scheduled_lengths | per resource class u (for W_u/P_u) |  |
| W_resource[exp] | `work.resource.exp` | `B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 21,760 | element-ops | partial | scheduled_lengths | per resource class u (for W_u/P_u) |  |
| exp_count_executed | `work.exp.executed` | `B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 20,480 | exp | partial | scheduled_lengths | exp over executed cells (masked included) | C_valid |
...
| N_Kproj[executed_graph] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
| N_Kproj | derived | complete w.r.t. the declared task/strategy/numerics scenario and this formula | `{"B": 2, "H": 4, "S_k": 64}` |  |
| N_Vproj[executed_graph] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
...
| F_total[executed_graph] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "D_n": 24, "D_r": 8, "D_v": 24, "Delta": 0, "H": 4, "R_k": 32, "R_q": 16, "S_k...` |  |
| F_projection[executed_graph] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "D_n": 24, "D_v": 24, "Delta": 0, "H": 4, "R_k": 32, "R_q": 16, "S_k": 64, "S_...` |  |
| F_attention[executed_graph] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "D_n": 24, "D_r": 8, "D_v": 24, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k...` |  |
...
| W_vec[mul] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "D_v": 24, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
| W_vec[mask] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
| W_vec[cmp] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
| W_vec[broadcast] | derived | incomplete: depends on undeclared/unbound broadcast_materialized (undeclared backend behaviour), scheduled_lengths | `{"B": 2, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
| W_vec[transpose] | derived | incomplete: depends on undeclared/unbound operand_transpose_materialized (undeclared backend behaviour), scheduled_lengths | `{"B": 2, "D_n": 24, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
| W_vec[exp] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
| W_vec[add] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "D_r": 8, "D_v": 24, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q...` |  |
| W_vec[div] | derived | incomplete: depends on undeclared/unbound final_normalize, scheduled_lengths | `{"B": 2, "D_v": 24, "H": 4, "S_q": 64}` |  |
| W_vec[recip] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{}` |  |
| W_vec[log] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "H": 4, "S_q": 64}` |  |
| W_vec[cast] | derived | incomplete: depends on undeclared/unbound cast_points, scheduled_lengths | `{"B": 2, "D_v": 24, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
| W_resource[vpu] | derived | incomplete: depends on undeclared/unbound cast_points, final_normalize, scheduled_lengths | `{"B": 2, "D_r": 8, "D_v": 24, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q...` |  |
| W_resource[reduce] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
| W_resource[layout] | derived | incomplete: depends on undeclared/unbound broadcast_materialized (undeclared backend behaviour), operand_transpose_materialized (undeclared backend behaviour), scheduled_lengths | `{"B": 2, "D_n": 24, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
| W_resource[exp] | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
| exp_count_executed | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
...
| metric | source | coverage | bindings | evidence |
|---|---|---|---|---|
| B[hbm_to_vmem] | conditional | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "D_n": 24, "D_r": 8, "D_v": 24, "Delta": 0, "H": 4, "R_k": 32, "R_q": 16, "S_k...` |  |
| B[vmem_to_hbm] | conditional | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "D_v": 24, "H": 4, "S_q": 64, "s_LSE": 4, "s_O": 2}` |  |
...
| layout[score_X] | `layout.score_X` | `L_l*L_s*s_X*ceiling(b_k/L_l)*ceiling(b_q/L_s) → 4*L_l*L_s*ceiling(16/L_l)*ceiling(16/L_s)` |  | byte | partial | L_l, L_s, layout.score_X | layout coverage, kind=vector_tiled, level=vreg (scenario: kind undeclared) | register-file capacity |
| layout[exp_E] | `layout.exp_E` | `L_l*L_s*s_E*ceiling(b_k/L_l)*ceiling(b_q/L_s) → 4*L_l*L_s*ceiling(16/L_l)*ceiling(16/L_s)` |  | byte | partial | L_l, L_s, layout.exp_E | layout coverage, kind=vector_tiled, level=vreg (scenario: kind undeclared) | register-file capacity |
...
| layout[accumulator_A] | `layout.accumulator_A` | `L_l*L_s*s_A*ceiling(D_v/L_l)*ceiling(b_q/L_s) → 4*L_l*L_s*ceiling(24/L_l)*ceiling(16/L_s)` |  | byte | partial | L_l, L_s, layout.accumulator_A | layout coverage, kind=vector_tiled, level=vreg (scenario: kind undeclared) | register-file capacity |
...
| layout[score_X] | conditional | incomplete: depends on undeclared/unbound L_l, L_s, layout.score_X | `{"b_k": 16, "b_q": 16, "s_X": 4}` |  |
| layout[exp_E] | conditional | incomplete: depends on undeclared/unbound L_l, L_s, layout.exp_E | `{"b_k": 16, "b_q": 16, "s_E": 4}` |  |
...
| layout[accumulator_A] | conditional | incomplete: depends on undeclared/unbound L_l, L_s, layout.accumulator_A | `{"D_v": 24, "b_q": 16, "s_A": 4}` |  |
...
| n_programs | `sched.programs` | `B*ceiling(H/h_pp)*ceiling(S_q/b_q)` | 32 | programs | partial | scheduled_lengths | (batch, head-group, q-block) programs under the declared loop nest |  |
| n_kv_block_visits | `sched.kv_visits` | `B*H*BlockStat(selected, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k)` | 80 | visits | partial | scheduled_lengths | selected (b, h, q-block, kv-block) rectangles |  |
...
| n_programs | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "H": 4, "S_q": 64, "b_q": 16, "h_pp": 1}` |  |
| n_kv_block_visits | derived | incomplete: depends on undeclared/unbound scheduled_lengths | `{"B": 2, "Delta": 0, "H": 4, "S_k": 64, "S_q": 64, "b_k": 16, "b_q": 16}` |  |
...
| metric | formula id | expression | value | unit | status | missing | scope | not equivalent to |
|---|---|---|---|---|---|---|---|---|
| T_LB[mxu] | `perf.lb.mxu` | `(2*B*D_n*H*R_k*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*D_n*H*R_q*S_q + 2*B*D_n*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*D_r*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*D_v*H*R_k*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*D_v*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k))/P_mxu` |  | s | unknown | P_mxu, scheduled_lengths | scope=None | actual latency; calibrated prediction |
| T_LB[vpu] | `perf.lb.vpu` | `(2*B*D_v*H*S_q + 2*B*D_v*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*H*S_q + B*H*BlockStat(masked_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + Piecewise((B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k), D_r > 0), (0, True)))/P_vpu` |  | s | unknown | P_vpu, scheduled_lengths | scope=None | actual latency; calibrated prediction |
| T_LB[reduce] | `perf.lb.reduce` | `(2*B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) - B*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k))/P_red` |  | s | unknown | P_red, scheduled_lengths | scope=None | actual latency; calibrated prediction |
| T_LB[layout] | `perf.lb.layout` | `(B*D_n*H*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k))/P_layout` |  | s | unknown | P_layout, scheduled_lengths | scope=None | actual latency; calibrated prediction |
| T_LB[exp] | `perf.lb.exp` | `(B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k))/P_exp` |  | s | unknown | P_exp, scheduled_lengths | scope=None | actual latency; calibrated prediction |
| T_LB[path:hbm_to_vmem] | `perf.lb.path:hbm_to_vmem` | `(B*D_n*H*R_k*s_Wk*ceiling(S_q/b_q) + B*D_n*H*R_q*s_Wq*ceiling(S_q/b_q) + B*D_r*H*S_q*s_Qr + B*D_r*s_Kr*ceiling(H/h_pp)*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*D_v*H*R_k*s_Wv*ceiling(S_q/b_q) + B*R_k*s_Ck*ceiling(H/h_pp)*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*R_q*S_q*s_Cq*ceiling(H/h_pp))/W_hbm` |  | s | unknown | W_hbm, scheduled_lengths | scope=None | actual latency; calibrated prediction |
| T_LB[path:vmem_to_hbm] | `perf.lb.path:vmem_to_hbm` | `(B*D_v*H*S_q*s_O + B*H*S_q*s_LSE)/W_hbm_w` |  | s | unknown | W_hbm_w, scheduled_lengths | scope=None | actual latency; calibrated prediction |
| T_LB[path:vmem_to_vreg] | `perf.lb.path:vmem_to_vreg` | `(B*D_n*H*b_k*s_Kn*BlockStat(selected, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*D_n*H*b_q*s_Qn*ceiling(S_q/b_q) + B*D_r*H*b_k*s_Kr*BlockStat(selected, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*D_r*H*b_q*s_Qr*ceiling(S_q/b_q) + B*D_v*H*b_k*s_V*BlockStat(selected, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k))/W_vmem` |  | s | unknown | W_vmem, q_resident, register_schedule_evidence, scheduled_lengths | scope=None | actual latency; calibrated prediction |
| T_LB[critical_path] | `perf.lb.critical_path` | `CP` |  | s | unknown | CP | scope=None | actual latency; calibrated prediction |
| T_LB[combined] | `perf.lb.combined` | `max((2*B*D_n*H*R_k*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*D_n*H*R_q*S_q + 2*B*D_n*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*D_r*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*D_v*H*R_k*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*D_v*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k))/P_mxu, (2*B*D_v*H*S_q + 2*B*D_v*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*H*S_q + B*H*BlockStat(masked_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + 2*B*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + Piecewise((B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k), D_r > 0), (0, True)))/P_vpu, (2*B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) - B*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k))/P_red, (B*D_n*H*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k))/P_layout, (B*H*BlockStat(rect_cells, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*H*BlockStat(row_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k))/P_exp, (B*D_n*H*R_k*s_Wk*ceiling(S_q/b_q) + B*D_n*H*R_q*s_Wq*ceiling(S_q/b_q) + B*D_r*H*S_q*s_Qr + B*D_r*s_Kr*ceiling(H/h_pp)*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*D_v*H*R_k*s_Wv*ceiling(S_q/b_q) + B*R_k*s_Ck*ceiling(H/h_pp)*BlockStat(key_visits, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*R_q*S_q*s_Cq*ceiling(H/h_pp))/W_hbm, (B*D_v*H*S_q*s_O + B*H*S_q*s_LSE)/W_hbm_w, (B*D_n*H*b_k*s_Kn*BlockStat(selected, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*D_n*H*b_q*s_Qn*ceiling(S_q/b_q) + B*D_r*H*b_k*s_Kr*BlockStat(selected, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k) + B*D_r*H*b_q*s_Qr*ceiling(S_q/b_q) + B*D_v*H*b_k*s_V*BlockStat(selected, causal, S_q, S_k, b_q, b_k, Delta, skip_future, 0, 0, S_q, S_k))/W_vmem, CP)` |  | s | unknown | CP, P_exp, P_layout, P_mxu, P_red, P_vpu, W_hbm, W_hbm_w, W_vmem, q_resident, register_schedule_evidence, scheduled_lengths | overlap model full_overlap_max | calibrated prediction; measured time |
...
| metric | source | coverage | bindings | evidence |
|---|---|---|---|---|
| T_LB[mxu] | derived | incomplete: depends on undeclared/unbound P_mxu, scheduled_lengths | `{}` |  |
| T_LB[vpu] | derived | incomplete: depends on undeclared/unbound P_vpu, scheduled_lengths | `{}` |  |
| T_LB[reduce] | derived | incomplete: depends on undeclared/unbound P_red, scheduled_lengths | `{}` |  |
| T_LB[layout] | derived | incomplete: depends on undeclared/unbound P_layout, scheduled_lengths | `{}` |  |
| T_LB[exp] | derived | incomplete: depends on undeclared/unbound P_exp, scheduled_lengths | `{}` |  |
| T_LB[path:hbm_to_vmem] | derived | incomplete: depends on undeclared/unbound W_hbm, scheduled_lengths | `{}` |  |
| T_LB[path:vmem_to_hbm] | derived | incomplete: depends on undeclared/unbound W_hbm_w, scheduled_lengths | `{}` |  |
| T_LB[path:vmem_to_vreg] | derived | incomplete: depends on undeclared/unbound W_vmem, q_resident, register_schedule_evidence, scheduled_lengths | `{}` |  |
| T_LB[critical_path] | derived | incomplete: depends on undeclared/unbound CP | `{}` |  |
| T_LB[combined] | derived | incomplete: depends on undeclared/unbound CP, P_exp, P_layout, P_mxu, P_red, P_vpu, W_hbm, W_hbm_w, W_vmem, q_resident, register_schedule_evidence, scheduled_lengths | `{}` |  |
