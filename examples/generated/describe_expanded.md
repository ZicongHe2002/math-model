# MLA forward — symbolic description (expanded)

- mode: **symbolic**  |  adapter: `seven_input_latent`  |  algorithm: `expanded`
...
- B: needed by 41 metric(s), e.g. ['B', 'F_Q[useful]', 'F_K[useful]']
...
- C_valid: needed by 10 metric(s), e.g. ['F_Q[useful]', 'F_K[useful]', 'F_V[useful]']
- D_n: needed by 81 metric(s), e.g. ['D_n', 'F_Q[useful]', 'F_K[useful]']
- D_r: needed by 55 metric(s), e.g. ['D_r', 'F_QK_r[useful]', 'F_total[useful]']
- D_v: needed by 90 metric(s), e.g. ['D_v', 'F_V[useful]', 'F_PV[useful]']
- Delta: needed by 1 metric(s), e.g. ['Delta']
- H: needed by 46 metric(s), e.g. ['H', 'F_Q[useful]', 'F_K[useful]']
...
- R_k: needed by 33 metric(s), e.g. ['R_k', 'F_K[useful]', 'F_V[useful]']
- R_q: needed by 26 metric(s), e.g. ['R_q', 'F_Q[useful]', 'F_total[useful]']
- S_k: needed by 15 metric(s), e.g. ['S_k_active', 'F_K[useful]', 'F_V[useful]']
- S_q: needed by 33 metric(s), e.g. ['S_q_active', 'F_Q[useful]', 'F_total[useful]']
...
- b_k: needed by 53 metric(s), e.g. ['B[vmem_to_vreg]', 'local[kv_latent_rank_tile]', 'local[k_nope_tile]']
...
- mask: needed by 38 metric(s), e.g. ['mask', 'rows_without_visible_keys', 'C_valid']
...
- rect_policy: needed by 35 metric(s), e.g. ['N_Kproj[executed_graph]', 'N_Vproj[executed_graph]', 'F_total[executed_graph]']
...
- s_Ck: needed by 16 metric(s), e.g. ['bytes[kv_latent]', 'M_in', 'B[hbm_to_vmem]']
...
- s_Kn: needed by 22 metric(s), e.g. ['HBM_mat[K^n,V]', 'M_mat[K^n,V]', 'B[vmem_to_vreg]']
...
- s_Qn: needed by 43 metric(s), e.g. ['HBM_mat[Q^n]', 'M_mat[Q^n]', 'B[hbm_to_vmem]']
- s_Qr: needed by 41 metric(s), e.g. ['bytes[q_pe]', 'M_in', 'B[hbm_to_vmem]']
- s_V: needed by 34 metric(s), e.g. ['HBM_mat[K^n,V]', 'M_mat[K^n,V]', 'B[vmem_to_vreg]']
- s_Wk: needed by 16 metric(s), e.g. ['bytes[w_k_nope]', 'M_in', 'B[hbm_to_vmem]']
- s_Wq: needed by 15 metric(s), e.g. ['bytes[w_q_nope]', 'M_in', 'B[hbm_to_vmem]']
- s_Wv: needed by 16 metric(s), e.g. ['bytes[w_v]', 'M_in', 'B[hbm_to_vmem]']
...
| metric | formula id | expression | value | unit | status | missing | scope | not equivalent to |
|---|---|---|---|---|---|---|---|---|
| F_Q[useful] | `work.expanded.F_Q` | `2*B*D_n*H*R_q*S_q → 2*B*D_n*H*R_q*S_q` |  | FLOP | symbolic | B, C_valid, D_n, H, R_q, S_q, projection_scope | useful (C = C_valid), FMA=2, projection rows from scope 'full (scenario: projection_scope undeclared)' |  |
| F_K[useful] | `work.expanded.F_K` | `2*B*D_n*H*R_k*S_k → 2*B*D_n*H*R_k*S_k` |  | FLOP | symbolic | B, C_valid, D_n, H, R_k, S_k, projection_scope | useful (C = C_valid), FMA=2, projection rows from scope 'full (scenario: projection_scope undeclared)' |  |
| F_V[useful] | `work.expanded.F_V` | `2*B*D_v*H*R_k*S_k → 2*B*D_v*H*R_k*S_k` |  | FLOP | symbolic | B, C_valid, D_v, H, R_k, S_k, projection_scope | useful (C = C_valid), FMA=2, projection rows from scope 'full (scenario: projection_scope undeclared)' |  |
| F_QK_n[useful] | `work.expanded.F_QK_n` | `2*C_valid*D_n → 2*C_valid*D_n` |  | FLOP | symbolic | C_valid, D_n, projection_scope | useful (C = C_valid), FMA=2, projection rows from scope 'full (scenario: projection_scope undeclared)' |  |
| F_QK_r[useful] | `work.expanded.F_QK_r` | `2*C_valid*D_r → 2*C_valid*D_r` |  | FLOP | symbolic | C_valid, D_r, projection_scope | useful (C = C_valid), FMA=2, projection rows from scope 'full (scenario: projection_scope undeclared)' |  |
| F_PV[useful] | `work.expanded.F_PV` | `2*C_valid*D_v → 2*C_valid*D_v` |  | FLOP | symbolic | C_valid, D_v, projection_scope | useful (C = C_valid), FMA=2, projection rows from scope 'full (scenario: projection_scope undeclared)' |  |
| F_total[useful] | `work.expanded.total` | `2*B*D_n*H*R_k*S_k + 2*B*D_n*H*R_q*S_q + 2*B*D_v*H*R_k*S_k + 2*C_valid*(D_n + D_r + D_v) → 2*B*D_n*H*R_k*S_k + 2*B*D_n*H*R_q*S_q + 2*B*D_v*H*R_k*S_k + 2*C_valid*(D_n + D_r + D_v)` |  | FLOP | symbolic | B, C_valid, D_n, D_r, D_v, H, R_k, R_q, S_k, S_q, projection_scope | useful mathematical work under projection scope 'full (scenario: projection_scope undeclared)' | scheduled work; compiled instruction count |
| F_E[spec_closed_form] | `work.expanded.closed_form` | `2*B*D_n*H*R_q*S_q + 2*B*H*R_k*S_k*(D_n + D_v) + 2*C_valid*(D_n + D_r + D_v) → 2*B*D_n*H*R_q*S_q + 2*B*H*R_k*S_k*(D_n + D_v) + 2*C_valid*(D_n + D_r + D_v)` |  | FLOP | symbolic | B, C_valid, D_n, D_r, D_v, H, R_k, R_q, S_k, S_q | specification boxed formula: every Q row and every K/V row projected once (full projection, no cache) |  |
...
| N_Kproj[executed_graph] | `work.projection.rows.N_Kproj.executed` | `H*N_keyvisits → H*N_keyvisits` |  | rows | symbolic | H, N_keyvisits, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | K rows projected in this invocation as the executed graph performs them, including the KV-loop multiplicity and excluding tiles no query block scans (spec 5.2) | N_Kproj, the once-per-row count of the declared scope |
| N_Kproj | `work.projection.rows.N_Kproj` | `B*H*S_k → B*H*S_k` |  | rows | symbolic | B, H, S_k, projection_scope | K rows projected in this invocation counted once per row, from the declared projection_scope and the variant's dataflow | the tensor's row count; rows read from HBM; the executed-graph projected-row count under in-loop projection |
| N_Vproj[executed_graph] | `work.projection.rows.N_Vproj.executed` | `H*N_keyvisits → H*N_keyvisits` |  | rows | symbolic | H, N_keyvisits, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | V rows projected in this invocation as the executed graph performs them, including the KV-loop multiplicity and excluding tiles no query block scans (spec 5.2) | N_Vproj, the once-per-row count of the declared scope |
| N_Vproj | `work.projection.rows.N_Vproj` | `B*H*S_k → B*H*S_k` |  | rows | symbolic | B, H, S_k, projection_scope | V rows projected in this invocation counted once per row, from the declared projection_scope and the variant's dataflow | the tensor's row count; rows read from HBM; the executed-graph projected-row count under in-loop projection |
| F_proj[general] | `work.projection.general` | `2*B*D_n*H*R_k*S_k + 2*B*D_n*H*R_q*S_q + 2*B*D_v*H*R_k*S_k → 2*B*D_n*H*R_k*S_k + 2*B*D_n*H*R_q*S_q + 2*B*D_v*H*R_k*S_k` |  | FLOP | symbolic | B, D_n, D_v, H, R_k, R_q, S_k, S_q, projection_scope | 2 N_Qproj R_q D_n + 2 N_Kproj R_k D_n + 2 N_Vproj R_k D_v with row counts from projection_scope | F_projection[executed_graph], which carries the loop multiplicity |
| F_total[executed_graph] | `work.expanded.graph.total` | `2*B*D_n*H*R_q*S_q + 2*C_pad*D_n + 2*C_pad*D_r + 2*C_pad*D_v + 2*D_n*H*N_keyvisits*R_k + 2*D_v*H*N_keyvisits*R_k → 2*B*D_n*H*R_q*S_q + 2*C_pad*D_n + 2*C_pad*D_r + 2*C_pad*D_v + 2*D_n*H*N_keyvisits*R_k + 2*D_v*H*N_keyvisits*R_k` |  | FLOP | symbolic | B, C_pad, D_n, D_r, D_v, H, N_keyvisits, R_k, R_q, S_q, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | graph sum of 2MNK*multiplicity over executed cells C_exec (declared extent policy) |  |
| F_projection[executed_graph] | `work.expanded.graph.projection` | `2*B*D_n*H*R_q*S_q + 2*D_n*H*N_keyvisits*R_k + 2*D_v*H*N_keyvisits*R_k → 2*B*D_n*H*R_q*S_q + 2*D_n*H*N_keyvisits*R_k + 2*D_v*H*N_keyvisits*R_k` |  | FLOP | symbolic | B, D_n, D_v, H, N_keyvisits, R_k, R_q, S_q, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | graph category |  |
| F_attention[executed_graph] | `work.expanded.graph.attention` | `2*C_pad*D_n + 2*C_pad*D_r + 2*C_pad*D_v → 2*C_pad*D_n + 2*C_pad*D_r + 2*C_pad*D_v` |  | FLOP | symbolic | C_pad, D_n, D_r, D_v, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | graph category |  |
...
| F_A_minus_F_E | `work.path_difference` | `-2*B*D_n*H*R_k*S_k + 2*B*D_n*H*R_k*S_q - 2*B*D_v*H*R_k*S_k + 2*B*D_v*H*R_k*S_q + 2*C_valid*(D_r + 2*R_k) - 2*C_valid*(D_n + D_r + D_v) → -2*B*D_n*H*R_k*S_k + 2*B*D_n*H*R_k*S_q - 2*B*D_v*H*R_k*S_k + 2*B*D_v*H*R_k*S_q + 2*C_valid*(D_r + 2*R_k) - 2*C_valid*(D_n + D_r + D_v)` |  | FLOP | symbolic | B, C_valid, D_n, D_r, D_v, H, R_k, S_k, S_q, projection_scope | absorbed two-step minus expanded, same projection rows and same C (useful) |  |
...
| W_vec[mul] | `work.vector.mul` | `C_pad + D_v*N_rowvisits + N_rowvisits → C_pad + D_v*N_rowvisits + N_rowvisits` |  | element-ops | symbolic | C_pad, D_v, N_rowvisits, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[mask] | `work.vector.mask` | `C_masked → C_masked` |  | element-ops | symbolic | C_masked, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[cmp] | `work.vector.cmp` | `C_pad → C_pad` |  | element-ops | symbolic | C_pad, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[broadcast] | `work.vector.broadcast` | `C_pad → C_pad` |  | element-ops | symbolic | C_pad, broadcast_materialized (undeclared backend behaviour), executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[transpose] | `work.vector.transpose` | `D_n*H*N_keyvisits → D_n*H*N_keyvisits` |  | element-ops | symbolic | D_n, H, N_keyvisits, executed_extent_policy, exp_base, mask, operand_transpose_materialized (undeclared backend behaviour), outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[exp] | `work.vector.exp` | `C_pad + N_rowvisits → C_pad + N_rowvisits` |  | element-ops | symbolic | C_pad, N_rowvisits, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[add] | `work.vector.add` | `B*H*S_q + C_pad + D_v*N_rowvisits + Piecewise((C_pad, D_r > 0), (0, True)) → B*H*S_q + C_pad + D_v*N_rowvisits + Piecewise((C_pad, D_r > 0), (0, True))` |  | element-ops | symbolic | B, C_pad, D_r, D_v, H, N_rowvisits, S_q, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[div] | `work.vector.div` | `B*D_v*H*S_q → B*D_v*H*S_q` |  | element-ops | symbolic | B, D_v, H, S_q, executed_extent_policy, exp_base, final_normalize, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[recip] | `work.vector.recip` | `0` | 0 | element-ops | partial | executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[log] | `work.vector.log` | `B*H*S_q → B*H*S_q` |  | element-ops | symbolic | B, H, S_q, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_vec[cast] | `work.vector.cast` | `B*D_v*H*S_q + C_pad → B*D_v*H*S_q + C_pad` |  | element-ops | symbolic | B, C_pad, D_v, H, S_q, accumulator_dtype, output_dtype, cast_points, executed_extent_policy, exp_base, exp_dtype, p_operand_dtype, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | logical count before redundancy elimination; not instructions | vector instruction count; MXU FLOPs |
| W_resource[vpu] | `work.resource.vpu` | `2*B*D_v*H*S_q + 2*B*H*S_q + C_masked + 2*C_pad + 2*D_v*N_rowvisits + 2*N_rowvisits + Piecewise((C_pad, D_r > 0), (0, True)) → 2*B*D_v*H*S_q + 2*B*H*S_q + C_masked + 2*C_pad + 2*D_v*N_rowvisits + 2*N_rowvisits + Piecewise((C_pad, D_r > 0), (0, True))` |  | element-ops | symbolic | B, C_masked, C_pad, D_r, D_v, H, N_rowvisits, S_q, accumulator_dtype, output_dtype, cast_points, executed_extent_policy, exp_base, exp_dtype, p_operand_dtype, final_normalize, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | per resource class u (for W_u/P_u) |  |
| W_resource[reduce] | `work.resource.reduce` | `2*C_pad - N_rowvisits → 2*C_pad - N_rowvisits` |  | element-ops | symbolic | C_pad, N_rowvisits, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | per resource class u (for W_u/P_u) |  |
| W_resource[layout] | `work.resource.layout` | `C_pad + D_n*H*N_keyvisits → C_pad + D_n*H*N_keyvisits` |  | element-ops | symbolic | C_pad, D_n, H, N_keyvisits, broadcast_materialized (undeclared backend behaviour), executed_extent_policy, exp_base, mask, operand_transpose_materialized (undeclared backend behaviour), outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | per resource class u (for W_u/P_u) |  |
| W_resource[exp] | `work.resource.exp` | `C_pad + N_rowvisits → C_pad + N_rowvisits` |  | element-ops | symbolic | C_pad, N_rowvisits, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | per resource class u (for W_u/P_u) |  |
| exp_count_executed | `work.exp.executed` | `C_pad → C_pad` |  | exp | symbolic | C_pad, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | exp over executed cells (masked included) | C_valid |
...
| F_K[useful] | derived | incomplete: depends on undeclared/unbound B, C_valid, D_n, H, R_k, S_k, projection_scope | `{}` |  |
| F_V[useful] | derived | incomplete: depends on undeclared/unbound B, C_valid, D_v, H, R_k, S_k, projection_scope | `{}` |  |
| F_QK_n[useful] | derived | incomplete: depends on undeclared/unbound C_valid, D_n, projection_scope | `{}` |  |
| F_QK_r[useful] | derived | incomplete: depends on undeclared/unbound C_valid, D_r, projection_scope | `{}` |  |
| F_PV[useful] | derived | incomplete: depends on undeclared/unbound C_valid, D_v, projection_scope | `{}` |  |
| F_total[useful] | derived | incomplete: depends on undeclared/unbound B, C_valid, D_n, D_r, D_v, H, R_k, R_q, S_k, S_q, projection_scope | `{}` |  |
| F_E[spec_closed_form] | derived | incomplete: depends on undeclared/unbound B, C_valid, D_n, D_r, D_v, H, R_k, R_q, S_k, S_q | `{}` |  |
| N_Qproj | derived | incomplete: depends on undeclared/unbound B, H, S_q, projection_scope | `{}` |  |
| N_Kproj[executed_graph] | derived | incomplete: depends on undeclared/unbound H, N_keyvisits, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | `{}` |  |
| N_Kproj | derived | incomplete: depends on undeclared/unbound B, H, S_k, projection_scope | `{}` |  |
| N_Vproj[executed_graph] | derived | incomplete: depends on undeclared/unbound H, N_keyvisits, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | `{}` |  |
| N_Vproj | derived | incomplete: depends on undeclared/unbound B, H, S_k, projection_scope | `{}` |  |
| F_proj[general] | derived | incomplete: depends on undeclared/unbound B, D_n, D_v, H, R_k, R_q, S_k, S_q, projection_scope | `{}` |  |
| F_total[executed_graph] | derived | incomplete: depends on undeclared/unbound B, C_pad, D_n, D_r, D_v, H, N_keyvisits, R_k, R_q, S_q, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | `{}` |  |
| F_projection[executed_graph] | derived | incomplete: depends on undeclared/unbound B, D_n, D_v, H, N_keyvisits, R_k, R_q, S_q, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | `{}` |  |
| F_attention[executed_graph] | derived | incomplete: depends on undeclared/unbound C_pad, D_n, D_r, D_v, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | `{}` |  |
...
| W_vec[mul] | derived | incomplete: depends on undeclared/unbound C_pad, D_v, N_rowvisits, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_vec[mask] | derived | incomplete: depends on undeclared/unbound C_masked, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_vec[cmp] | derived | incomplete: depends on undeclared/unbound C_pad, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_vec[broadcast] | derived | incomplete: depends on undeclared/unbound C_pad, broadcast_materialized (undeclared backend behaviour), executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_vec[transpose] | derived | incomplete: depends on undeclared/unbound D_n, H, N_keyvisits, executed_extent_policy, exp_base, mask, operand_transpose_materialized (undeclared backend behaviour), outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_vec[exp] | derived | incomplete: depends on undeclared/unbound C_pad, N_rowvisits, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_vec[add] | derived | incomplete: depends on undeclared/unbound B, C_pad, D_r, D_v, H, N_rowvisits, S_q, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_vec[div] | derived | incomplete: depends on undeclared/unbound B, D_v, H, S_q, executed_extent_policy, exp_base, final_normalize, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_vec[recip] | derived | incomplete: depends on undeclared/unbound executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_vec[log] | derived | incomplete: depends on undeclared/unbound B, H, S_q, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_vec[cast] | derived | incomplete: depends on undeclared/unbound B, C_pad, D_v, H, S_q, accumulator_dtype, output_dtype, cast_points, executed_extent_policy, exp_base, exp_dtype, p_operand_dtype, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_resource[vpu] | derived | incomplete: depends on undeclared/unbound B, C_masked, C_pad, D_r, D_v, H, N_rowvisits, S_q, accumulator_dtype, output_dtype, cast_points, executed_extent_policy, exp_base, exp_dtype, p_operand_dtype, final_normalize, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_resource[reduce] | derived | incomplete: depends on undeclared/unbound C_pad, N_rowvisits, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_resource[layout] | derived | incomplete: depends on undeclared/unbound C_pad, D_n, H, N_keyvisits, broadcast_materialized (undeclared backend behaviour), executed_extent_policy, exp_base, mask, operand_transpose_materialized (undeclared backend behaviour), outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| W_resource[exp] | derived | incomplete: depends on undeclared/unbound C_pad, N_rowvisits, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| exp_count_executed | derived | incomplete: depends on undeclared/unbound C_pad, executed_extent_policy, mask, projection_in_kv_loop, projection_scope, rect_policy, schedule | `{}` |  |
...
| HBM_mat[K^n,V] | `bytes.mat.mat_expanded_kv` | `B*D_n*H*S_k*s_Kn + B*D_v*H*S_k*s_V + D_n*H*N_keyvisits*s_Kn + D_v*H*N_keyvisits*s_V → B*D_n*H*S_k*s_Kn + B*D_v*H*S_k*s_V + D_n*H*N_keyvisits*s_Kn + D_v*H*N_keyvisits*s_V` |  | byte | symbolic | B, D_n, D_v, H, N_keyvisits, S_k, materialize.expanded_kv, s_Kn, s_V | write + read traffic if materialized (read under the declared scan policy) | interface bytes |
| M_mat[K^n,V] | `bytes.mat.size.mat_expanded_kv` | `B*H*S_k*(D_n*s_Kn + D_v*s_V) → B*H*S_k*(D_n*s_Kn + D_v*s_V)` |  | byte | symbolic | B, D_n, D_v, H, S_k, materialize.expanded_kv, s_Kn, s_V | logical size of the materialized intermediate itself (spec 6.2 M^mat) | its HBM traffic; interface bytes |
| HBM_mat[Q^n] | `bytes.mat.mat_q_nope` | `2*B*D_n*H*S_q*s_Qn → 2*B*D_n*H*S_q*s_Qn` |  | byte | symbolic | B, D_n, H, S_q, materialize.q_nope, s_Qn | write + read traffic if materialized (read under the declared scan policy) | interface bytes |
| M_mat[Q^n] | `bytes.mat.size.mat_q_nope` | `B*D_n*H*S_q*s_Qn → B*D_n*H*S_q*s_Qn` |  | byte | symbolic | B, D_n, H, S_q, materialize.q_nope, s_Qn | logical size of the materialized intermediate itself (spec 6.2 M^mat) | its HBM traffic; interface bytes |
...
| HBM_mat[K^n,V] | conditional | incomplete: depends on undeclared/unbound B, D_n, D_v, H, N_keyvisits, S_k, materialize.expanded_kv, s_Kn, s_V | `{}` |  |
| M_mat[K^n,V] | conditional | incomplete: depends on undeclared/unbound B, D_n, D_v, H, S_k, materialize.expanded_kv, s_Kn, s_V | `{}` |  |
| HBM_mat[Q^n] | conditional | incomplete: depends on undeclared/unbound B, D_n, H, S_q, materialize.q_nope, s_Qn | `{}` |  |
| M_mat[Q^n] | conditional | incomplete: depends on undeclared/unbound B, D_n, H, S_q, materialize.q_nope, s_Qn | `{}` |  |
...
| metric | formula id | expression | value | unit | status | missing | scope | not equivalent to |
|---|---|---|---|---|---|---|---|---|
| B[hbm_to_vmem] | `bytes.path.hbm_to_vmem` | `B*D_n*H*R_k*s_Wk*ceiling(S_q/b_q) + B*D_n*H*R_q*s_Wq*ceiling(S_q/b_q) + B*D_n*H*S_q*s_Qn + B*D_r*H*S_q*s_Qr + B*D_v*H*R_k*s_Wv*ceiling(S_q/b_q) + B*R_q*S_q*s_Cq*ceiling(H/h_pp) + D_r*N_keyvisits*s_Kr*ceiling(H/h_pp) + N_keyvisits*R_k*s_Ck*ceiling(H/h_pp) → B*D_n*H*R_k*s_Wk*ceiling(S_q/b_q) + B*D_n*H*R_q*s_Wq*ceiling(S_q/b_q) + B*D_n*H*S_q*s_Qn + B*D_r*H*S_q*s_Qr + B*D_v*H*R_k*s_Wv*ceiling(S_q/b_q) + B*R_q*S_q*s_Cq*ceiling(H/h_pp) + D_r*N_keyvisits*s_Kr*ceiling(H/h_pp) + N_keyvisits*R_k*s_Ck*ceiling(H/h_pp)` |  | byte | symbolic | B, D_n, D_r, D_v, H, N_keyvisits, R_k, R_q, S_q, b_q, executed_extent_policy, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, outputs, projection_scope, rect_policy, s_Ck, s_Cq, s_Kr, s_Qn, s_Qr, s_Wk, s_Wq, s_Wv, schedule | sum_r m_r n_r over the scenario's transfer events | physical HBM traffic with caching; spill traffic |
| B[vmem_to_hbm] | `bytes.path.vmem_to_hbm` | `B*D_n*H*S_q*s_Qn + B*D_v*H*S_q*s_O + B*H*S_q*s_LSE → B*D_n*H*S_q*s_Qn + B*D_v*H*S_q*s_O + B*H*S_q*s_LSE` |  | byte | symbolic | B, D_n, D_v, H, S_q, executed_extent_policy, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, outputs, projection_scope, rect_policy, s_LSE, s_O, s_Qn, schedule | sum_r m_r n_r over the scenario's transfer events | physical HBM traffic with caching; spill traffic |
| B[vmem_to_vreg] | `bytes.path.vmem_to_vreg` | `B*D_n*H*b_q*s_Qn*ceiling(S_q/b_q) + B*D_r*H*b_q*s_Qr*ceiling(S_q/b_q) + D_n*N_kvblocks*b_k*s_Kn + D_r*N_kvblocks*b_k*s_Kr + D_v*N_kvblocks*b_k*s_V → B*D_n*H*b_q*s_Qn*ceiling(S_q/b_q) + B*D_r*H*b_q*s_Qr*ceiling(S_q/b_q) + D_n*N_kvblocks*b_k*s_Kn + D_r*N_kvblocks*b_k*s_Kr + D_v*N_kvblocks*b_k*s_V` |  | byte | symbolic | B, D_n, D_r, D_v, H, N_kvblocks, S_q, b_k, b_q, executed_extent_policy, heads_per_program, mask, q_resident, rect_policy, register_schedule_evidence, s_Kn, s_Kr, s_Qn, s_Qr, s_V | sum_r m_r n_r over the scenario's transfer events | physical HBM traffic with caching; spill traffic |
...
| metric | source | coverage | bindings | evidence |
|---|---|---|---|---|
| B[hbm_to_vmem] | conditional | incomplete: depends on undeclared/unbound B, D_n, D_r, D_v, H, N_keyvisits, R_k, R_q, S_q, b_q, executed_extent_policy, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, outputs, projection_scope, rect_policy, s_Ck, s_Cq, s_Kr, s_Qn, s_Qr, s_Wk, s_Wq, s_Wv, schedule | `{}` |  |
| B[vmem_to_hbm] | conditional | incomplete: depends on undeclared/unbound B, D_n, D_v, H, S_q, executed_extent_policy, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, outputs, projection_scope, rect_policy, s_LSE, s_O, s_Qn, schedule | `{}` |  |
| B[vmem_to_vreg] | conditional | incomplete: depends on undeclared/unbound B, D_n, D_r, D_v, H, N_kvblocks, S_q, b_k, b_q, executed_extent_policy, heads_per_program, mask, q_resident, rect_policy, register_schedule_evidence, s_Kn, s_Kr, s_Qn, s_Qr, s_V | `{}` |  |
...
| local[q_proj_acc] | `local.q_proj_acc.bytes` | `D_n*b_q*s_Qacc → D_n*b_q*s_Qacc` |  | byte | symbolic | D_n, b_q, s_Qacc | one logical object; level=vreg; live in ['q_proj'] | VREG allocation; spill traffic |
| local[q_nope_operand] | `local.q_nope_operand.bytes` | `D_n*b_q*s_Qn → D_n*b_q*s_Qn` |  | byte | symbolic | D_n, b_q, q_resident, s_Qn | one logical object; level=vmem; live in ['kv_proj', 'score', 'softmax', 'pv', 'finalize'] | VREG allocation; spill traffic |
| local[kv_latent_rank_tile] | `local.kv_latent_rank_tile.bytes` | `R_k*b_k*n_buf*s_Ck → R_k*b_k*n_buf*s_Ck` |  | byte | symbolic | R_k, b_k, b_rk, kv_buffers, materialize.expanded_kv, s_Ck | one logical object x its buffer count; level=vmem; live in ['kv_proj']; residency window of n_buf=n_buf buffers (7.3 allocation, not the 7.1 logical size) | VREG allocation; spill traffic; the 7.1 logical size of one buffer |
| local[wk_rank_tile] | `local.wk_rank_tile.bytes` | `D_n*R_k*s_Wk → D_n*R_k*s_Wk` |  | byte | symbolic | D_n, R_k, b_rk, materialize.expanded_kv, s_Wk | one logical object; level=vmem; live in ['kv_proj'] | VREG allocation; spill traffic |
| local[wv_rank_tile] | `local.wv_rank_tile.bytes` | `D_v*R_k*s_Wv → D_v*R_k*s_Wv` |  | byte | symbolic | D_v, R_k, b_rk, materialize.expanded_kv, s_Wv | one logical object; level=vmem; live in ['kv_proj'] | VREG allocation; spill traffic |
| local[k_nope_tile] | `local.k_nope_tile.bytes` | `D_n*b_k*s_Kn → D_n*b_k*s_Kn` |  | byte | symbolic | D_n, b_k, s_Kn | one logical object; level=vmem; live in ['kv_proj', 'score'] | VREG allocation; spill traffic |
| local[v_tile] | `local.v_tile.bytes` | `D_v*b_k*s_V → D_v*b_k*s_V` |  | byte | symbolic | D_v, b_k, s_V, v_load | one logical object; level=vmem; live in ['kv_proj', 'score', 'softmax', 'pv'] | VREG allocation; spill traffic |
...
| local[accumulator_A] | `local.accumulator_A.bytes` | `D_v*b_q*s_A → D_v*b_q*s_A` |  | byte | symbolic | D_v, b_q, s_A | one logical object; level=vreg; live in ['kv_proj', 'score', 'softmax', 'pv', 'finalize'] | VREG allocation; spill traffic |
...
| local[q_pe_tile] | `local.q_pe_tile.bytes` | `D_r*b_q*s_Qr → D_r*b_q*s_Qr` |  | byte | symbolic | D_r, b_q, q_resident, s_Qr | one logical object; level=vmem; live in ['kv_proj', 'score', 'softmax', 'pv', 'finalize'] | VREG allocation; spill traffic |
...
| layout[q_nope_operand] | `layout.q_nope_operand` | `L_l*L_s*s_Qn*ceiling(D_n/L_l)*ceiling(b_q/L_s) → L_l*L_s*s_Qn*ceiling(D_n/L_l)*ceiling(b_q/L_s)` |  | byte | symbolic | D_n, L_l, L_s, b_q, layout.q_nope_operand, s_Qn | layout coverage, kind=vector_tiled, level=vmem (scenario: kind undeclared) | register-file capacity |
| layout[kv_latent_rank_tile] | `layout.kv_latent_rank_tile` | `L_l*L_s*n_buf*s_Ck*ceiling(R_k/L_l)*ceiling(b_k/L_s) → L_l*L_s*n_buf*s_Ck*ceiling(R_k/L_l)*ceiling(b_k/L_s)` |  | byte | symbolic | L_l, L_s, R_k, b_k, kv_buffers, layout.kv_latent_rank_tile, s_Ck | layout coverage, kind=vector_tiled, level=vmem (scenario: kind undeclared) | register-file capacity |
| layout[wk_rank_tile] | `layout.wk_rank_tile` | `L_l*L_s*s_Wk*ceiling(D_n/L_l)*ceiling(R_k/L_s) → L_l*L_s*s_Wk*ceiling(D_n/L_l)*ceiling(R_k/L_s)` |  | byte | symbolic | D_n, L_l, L_s, R_k, layout.wk_rank_tile, s_Wk | layout coverage, kind=vector_tiled, level=vmem (scenario: kind undeclared) | register-file capacity |
| layout[wv_rank_tile] | `layout.wv_rank_tile` | `L_l*L_s*s_Wv*ceiling(D_v/L_l)*ceiling(R_k/L_s) → L_l*L_s*s_Wv*ceiling(D_v/L_l)*ceiling(R_k/L_s)` |  | byte | symbolic | D_v, L_l, L_s, R_k, layout.wv_rank_tile, s_Wv | layout coverage, kind=vector_tiled, level=vmem (scenario: kind undeclared) | register-file capacity |
| layout[k_nope_tile] | `layout.k_nope_tile` | `L_l*L_s*s_Kn*ceiling(D_n/L_l)*ceiling(b_k/L_s) → L_l*L_s*s_Kn*ceiling(D_n/L_l)*ceiling(b_k/L_s)` |  | byte | symbolic | D_n, L_l, L_s, b_k, layout.k_nope_tile, s_Kn | layout coverage, kind=vector_tiled, level=vmem (scenario: kind undeclared) | register-file capacity |
| layout[v_tile] | `layout.v_tile` | `L_l*L_s*s_V*ceiling(D_v/L_l)*ceiling(b_k/L_s) → L_l*L_s*s_V*ceiling(D_v/L_l)*ceiling(b_k/L_s)` |  | byte | symbolic | D_v, L_l, L_s, b_k, layout.v_tile, s_V | layout coverage, kind=vector_tiled, level=vmem (scenario: kind undeclared) | register-file capacity |
...
| layout[accumulator_A] | `layout.accumulator_A` | `L_l*L_s*s_A*ceiling(D_v/L_l)*ceiling(b_q/L_s) → L_l*L_s*s_A*ceiling(D_v/L_l)*ceiling(b_q/L_s)` |  | byte | symbolic | D_v, L_l, L_s, b_q, layout.accumulator_A, s_A | layout coverage, kind=vector_tiled, level=vreg (scenario: kind undeclared) | register-file capacity |
...
| local[q_nope_operand] | derived | incomplete: depends on undeclared/unbound D_n, b_q, q_resident, s_Qn | `{}` |  |
| local[kv_latent_rank_tile] | derived | incomplete: depends on undeclared/unbound R_k, b_k, b_rk, kv_buffers, materialize.expanded_kv, s_Ck | `{}` |  |
| local[wk_rank_tile] | derived | incomplete: depends on undeclared/unbound D_n, R_k, b_rk, materialize.expanded_kv, s_Wk | `{}` |  |
| local[wv_rank_tile] | derived | incomplete: depends on undeclared/unbound D_v, R_k, b_rk, materialize.expanded_kv, s_Wv | `{}` |  |
| local[k_nope_tile] | derived | incomplete: depends on undeclared/unbound D_n, b_k, s_Kn | `{}` |  |
| local[v_tile] | derived | incomplete: depends on undeclared/unbound D_v, b_k, s_V, v_load | `{}` |  |
...
| local[accumulator_A] | derived | incomplete: depends on undeclared/unbound D_v, b_q, s_A | `{}` |  |
...
| layout[q_nope_operand] | conditional | incomplete: depends on undeclared/unbound D_n, L_l, L_s, b_q, layout.q_nope_operand, s_Qn | `{}` |  |
| layout[kv_latent_rank_tile] | conditional | incomplete: depends on undeclared/unbound L_l, L_s, R_k, b_k, kv_buffers, layout.kv_latent_rank_tile, s_Ck | `{}` |  |
| layout[wk_rank_tile] | conditional | incomplete: depends on undeclared/unbound D_n, L_l, L_s, R_k, layout.wk_rank_tile, s_Wk | `{}` |  |
| layout[wv_rank_tile] | conditional | incomplete: depends on undeclared/unbound D_v, L_l, L_s, R_k, layout.wv_rank_tile, s_Wv | `{}` |  |
| layout[k_nope_tile] | conditional | incomplete: depends on undeclared/unbound D_n, L_l, L_s, b_k, layout.k_nope_tile, s_Kn | `{}` |  |
| layout[v_tile] | conditional | incomplete: depends on undeclared/unbound D_v, L_l, L_s, b_k, layout.v_tile, s_V | `{}` |  |
...
| layout[accumulator_A] | conditional | incomplete: depends on undeclared/unbound D_v, L_l, L_s, b_q, layout.accumulator_A, s_A | `{}` |  |
...
| M_live[all:kv_proj] | `pressure.live.all.kv_proj` | `D_n*R_k*s_Wk + D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*R_k*s_Wv + D_v*b_k*s_V + D_v*b_q*s_A + R_k*b_k*n_buf*s_Ck + 2*b_q*s_state + s_scratch → D_n*R_k*s_Wk + D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*R_k*s_Wv + D_v*b_k*s_V + D_v*b_q*s_A + R_k*b_k*n_buf*s_Ck + 2*b_q*s_state + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, R_k, b_k, b_q, b_rk, exp_alias_p_operand, kv_buffers, materialize.expanded_kv, q_resident, s_A, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, s_state, score_alias_exp, scratch_bytes, v_load | source-level live set at this stage cut, level=all (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[all:score] | `pressure.live.all.score` | `D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_k*n_buf*s_Kr + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + 2*b_k*b_q*s_X + 2*b_q*s_state + s_scratch → D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_k*n_buf*s_Kr + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + 2*b_k*b_q*s_X + 2*b_q*s_state + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, kv_buffers, s_A, s_Kn, s_Kr, s_Qn, s_Qr, s_V, s_X, s_state, score_alias_exp, scratch_bytes, v_load | source-level live set at this stage cut, level=all (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[all:softmax] | `pressure.live.all.softmax` | `D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_X + 3*b_q*s_state + s_scratch → D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_X + 3*b_q*s_state + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, exp_alias_p_operand, q_resident, s_A, s_E, s_Qn, s_Qr, s_V, s_X, s_state, score_alias_exp, scratch_bytes, v_load | source-level live set at this stage cut, level=all (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[all:pv] | `pressure.live.all.pv` | `D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_P + 3*b_q*s_state + s_scratch → D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_P + 3*b_q*s_state + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, exp_alias_p_operand, q_resident, s_A, s_E, s_P, s_Qn, s_Qr, s_V, s_state, score_alias_exp, scratch_bytes | source-level live set at this stage cut, level=all (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[all:finalize] | `pressure.live.all.finalize` | `D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_q*s_A + D_v*b_q*s_O + b_q*s_LSE + 2*b_q*s_state + s_scratch → D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_q*s_A + D_v*b_q*s_O + b_q*s_LSE + 2*b_q*s_state + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_q, exp_alias_p_operand, outputs, q_resident, s_A, s_LSE, s_O, s_Qn, s_Qr, s_state, score_alias_exp, scratch_bytes | source-level live set at this stage cut, level=all (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_peak[all] | `pressure.peak.all` | `Max(D_n*R_q*s_Wq + D_n*b_q*s_Qacc + R_q*b_q*s_Cq + s_scratch, D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_q*s_A + D_v*b_q*s_O + b_q*s_LSE + 2*b_q*s_state + s_scratch, D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_P + 3*b_q*s_state + s_scratch, D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_X + 3*b_q*s_state + s_scratch, D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_k*n_buf*s_Kr + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + 2*b_k*b_q*s_X + 2*b_q*s_state + s_scratch, D_n*R_k*s_Wk + D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*R_k*s_Wv + D_v*b_k*s_V + D_v*b_q*s_A + R_k*b_k*n_buf*s_Ck + 2*b_q*s_state + s_scratch) → Max(D_n*R_q*s_Wq + D_n*b_q*s_Qacc + R_q*b_q*s_Cq + s_scratch, D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_q*s_A + D_v*b_q*s_O + b_q*s_LSE + 2*b_q*s_state + s_scratch, D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_P + 3*b_q*s_state + s_scratch, D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_X + 3*b_q*s_state + s_scratch, D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_k*n_buf*s_Kr + D_r*b_q*s_Qr + D_v*b_k*s_V + D_v*b_q*s_A + 2*b_k*b_q*s_X + 2*b_q*s_state + s_scratch, D_n*R_k*s_Wk + D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*R_k*s_Wv + D_v*b_k*s_V + D_v*b_q*s_A + R_k*b_k*n_buf*s_Ck + 2*b_q*s_state + s_scratch)` |  | byte | symbolic | D_n, D_r, D_v, R_k, R_q, b_k, b_q, b_rk, b_rq, both_qk_branches_live and D_r > 0, exp_alias_p_operand, kv_buffers, materialize.expanded_kv, outputs, q_resident, s_A, s_Ck, s_Cq, s_E, s_Kn, s_Kr, s_LSE, s_O, s_P, s_Qacc, s_Qn, s_Qr, s_V, s_Wk, s_Wq, s_Wv, s_X, s_state, score_alias_exp, scratch_bytes, v_load | max over stage cuts, level=all (source level) | actual spill; OOM proof |
...
| M_live[vreg:kv_proj] | `pressure.live.vreg.kv_proj` | `D_v*b_q*s_A + 2*b_q*s_state → D_v*b_q*s_A + 2*b_q*s_state` |  | byte | symbolic | D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp | source-level live set at this stage cut, level=vreg (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[vreg:score] | `pressure.live.vreg.score` | `D_v*b_q*s_A + 2*b_k*b_q*s_X + 2*b_q*s_state → D_v*b_q*s_A + 2*b_k*b_q*s_X + 2*b_q*s_state` |  | byte | symbolic | D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, s_A, s_X, s_state, score_alias_exp | source-level live set at this stage cut, level=vreg (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[vreg:softmax] | `pressure.live.vreg.softmax` | `D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_X + 3*b_q*s_state → D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_X + 3*b_q*s_state` |  | byte | symbolic | D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_X, s_state, score_alias_exp | source-level live set at this stage cut, level=vreg (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[vreg:pv] | `pressure.live.vreg.pv` | `D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_P + 3*b_q*s_state → D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_P + 3*b_q*s_state` |  | byte | symbolic | D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_P, s_state, score_alias_exp | source-level live set at this stage cut, level=vreg (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[vreg:finalize] | `pressure.live.vreg.finalize` | `D_v*b_q*s_A + 2*b_q*s_state → D_v*b_q*s_A + 2*b_q*s_state` |  | byte | symbolic | D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp | source-level live set at this stage cut, level=vreg (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_peak[vreg] | `pressure.peak.vreg` | `Max(D_n*b_q*s_Qacc, D_v*b_q*s_A + 2*b_k*b_q*s_X + 2*b_q*s_state, D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_P + 3*b_q*s_state, D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_X + 3*b_q*s_state) → Max(D_n*b_q*s_Qacc, D_v*b_q*s_A + 2*b_k*b_q*s_X + 2*b_q*s_state, D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_P + 3*b_q*s_state, D_v*b_q*s_A + b_k*b_q*s_E + b_k*b_q*s_X + 3*b_q*s_state)` |  | byte | symbolic | D_n, D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, s_A, s_E, s_P, s_Qacc, s_X, s_state, score_alias_exp | max over stage cuts, level=vreg (source level) | actual spill; OOM proof |
...
| M_live[vmem:kv_proj] | `pressure.live.vmem.kv_proj` | `D_n*R_k*s_Wk + D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*R_k*s_Wv + D_v*b_k*s_V + R_k*b_k*n_buf*s_Ck + s_scratch → D_n*R_k*s_Wk + D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*R_k*s_Wv + D_v*b_k*s_V + R_k*b_k*n_buf*s_Ck + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, R_k, b_k, b_q, b_rk, kv_buffers, materialize.expanded_kv, q_resident, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, scratch_bytes, v_load | source-level live set at this stage cut, level=vmem (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[vmem:score] | `pressure.live.vmem.score` | `D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_k*n_buf*s_Kr + D_r*b_q*s_Qr + D_v*b_k*s_V + s_scratch → D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_k*n_buf*s_Kr + D_r*b_q*s_Qr + D_v*b_k*s_V + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, kv_buffers, s_Kn, s_Kr, s_Qn, s_Qr, s_V, scratch_bytes, v_load | source-level live set at this stage cut, level=vmem (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[vmem:softmax] | `pressure.live.vmem.softmax` | `D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + s_scratch → D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, v_load | source-level live set at this stage cut, level=vmem (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[vmem:pv] | `pressure.live.vmem.pv` | `D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + s_scratch → D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_k*s_V + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes | source-level live set at this stage cut, level=vmem (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_live[vmem:finalize] | `pressure.live.vmem.finalize` | `D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_q*s_O + b_q*s_LSE + s_scratch → D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_q*s_O + b_q*s_LSE + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_q, outputs, q_resident, s_LSE, s_O, s_Qn, s_Qr, scratch_bytes | source-level live set at this stage cut, level=vmem (aliases deduplicated) | compiler register allocation; VMEM allocation |
| M_peak[vmem] | `pressure.peak.vmem` | `Max(D_n*R_q*s_Wq + R_q*b_q*s_Cq + s_scratch, D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_q*s_O + b_q*s_LSE + s_scratch, D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_k*n_buf*s_Kr + D_r*b_q*s_Qr + D_v*b_k*s_V + s_scratch, D_n*R_k*s_Wk + D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*R_k*s_Wv + D_v*b_k*s_V + R_k*b_k*n_buf*s_Ck + s_scratch) → Max(D_n*R_q*s_Wq + R_q*b_q*s_Cq + s_scratch, D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*b_q*s_O + b_q*s_LSE + s_scratch, D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_k*n_buf*s_Kr + D_r*b_q*s_Qr + D_v*b_k*s_V + s_scratch, D_n*R_k*s_Wk + D_n*b_k*s_Kn + D_n*b_q*s_Qn + D_r*b_q*s_Qr + D_v*R_k*s_Wv + D_v*b_k*s_V + R_k*b_k*n_buf*s_Ck + s_scratch)` |  | byte | symbolic | D_n, D_r, D_v, R_k, R_q, b_k, b_q, b_rk, b_rq, kv_buffers, materialize.expanded_kv, outputs, q_resident, s_Ck, s_Cq, s_Kn, s_Kr, s_LSE, s_O, s_Qn, s_Qr, s_V, s_Wk, s_Wq, s_Wv, scratch_bytes, v_load | max over stage cuts, level=vmem (source level) | actual spill; OOM proof |
...
| envelope[all:kv_proj] | `pressure.envelope.all.kv_proj` | `D_n*R_k*s_Wk + D_v*R_k*s_Wv + b_k*(D_n*s_Kn + D_v*s_V + R_k*n_buf*s_Ck) + b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_A + 2*s_state) + s_scratch → D_n*R_k*s_Wk + D_v*R_k*s_Wv + b_k*(D_n*s_Kn + D_v*s_V + R_k*n_buf*s_Ck) + b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_A + 2*s_state) + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, R_k, b_k, b_q, b_rk, exp_alias_p_operand, kv_buffers, materialize.expanded_kv, q_resident, s_A, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, s_state, score_alias_exp, scratch_bytes, v_load | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=all | measured spill; spill probability |
| envelope[all:score] | `pressure.envelope.all.score` | `2*b_k*b_q*s_X + b_k*(D_n*s_Kn + D_r*n_buf*s_Kr + D_v*s_V) + b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_A + 2*s_state) + s_scratch → 2*b_k*b_q*s_X + b_k*(D_n*s_Kn + D_r*n_buf*s_Kr + D_v*s_V) + b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_A + 2*s_state) + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, kv_buffers, s_A, s_Kn, s_Kr, s_Qn, s_Qr, s_V, s_X, s_state, score_alias_exp, scratch_bytes, v_load | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=all | measured spill; spill probability |
| envelope[all:softmax] | `pressure.envelope.all.softmax` | `D_v*b_k*s_V + b_k*b_q*(s_E + s_X) + b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_A + 3*s_state) + s_scratch → D_v*b_k*s_V + b_k*b_q*(s_E + s_X) + b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_A + 3*s_state) + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, exp_alias_p_operand, q_resident, s_A, s_E, s_Qn, s_Qr, s_V, s_X, s_state, score_alias_exp, scratch_bytes, v_load | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=all | measured spill; spill probability |
| envelope[all:pv] | `pressure.envelope.all.pv` | `D_v*b_k*s_V + b_k*b_q*(s_E + s_P) + b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_A + 3*s_state) + s_scratch → D_v*b_k*s_V + b_k*b_q*(s_E + s_P) + b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_A + 3*s_state) + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, exp_alias_p_operand, q_resident, s_A, s_E, s_P, s_Qn, s_Qr, s_V, s_state, score_alias_exp, scratch_bytes | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=all | measured spill; spill probability |
| envelope[all:finalize] | `pressure.envelope.all.finalize` | `b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_A + D_v*s_O + s_LSE + 2*s_state) + s_scratch → b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_A + D_v*s_O + s_LSE + 2*s_state) + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_q, exp_alias_p_operand, outputs, q_resident, s_A, s_LSE, s_O, s_Qn, s_Qr, s_state, score_alias_exp, scratch_bytes | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=all | measured spill; spill probability |
...
| envelope[vreg:kv_proj] | `pressure.envelope.vreg.kv_proj` | `b_q*(D_v*s_A + 2*s_state) → b_q*(D_v*s_A + 2*s_state)` |  | byte | symbolic | D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=vreg | measured spill; spill probability |
| envelope[vreg:score] | `pressure.envelope.vreg.score` | `2*b_k*b_q*s_X + b_q*(D_v*s_A + 2*s_state) → 2*b_k*b_q*s_X + b_q*(D_v*s_A + 2*s_state)` |  | byte | symbolic | D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, s_A, s_X, s_state, score_alias_exp | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=vreg | measured spill; spill probability |
| envelope[vreg:softmax] | `pressure.envelope.vreg.softmax` | `b_k*b_q*(s_E + s_X) + b_q*(D_v*s_A + 3*s_state) → b_k*b_q*(s_E + s_X) + b_q*(D_v*s_A + 3*s_state)` |  | byte | symbolic | D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_X, s_state, score_alias_exp | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=vreg | measured spill; spill probability |
| envelope[vreg:pv] | `pressure.envelope.vreg.pv` | `b_k*b_q*(s_E + s_P) + b_q*(D_v*s_A + 3*s_state) → b_k*b_q*(s_E + s_P) + b_q*(D_v*s_A + 3*s_state)` |  | byte | symbolic | D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_P, s_state, score_alias_exp | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=vreg | measured spill; spill probability |
| envelope[vreg:finalize] | `pressure.envelope.vreg.finalize` | `b_q*(D_v*s_A + 2*s_state) → b_q*(D_v*s_A + 2*s_state)` |  | byte | symbolic | D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=vreg | measured spill; spill probability |
...
| envelope[vmem:kv_proj] | `pressure.envelope.vmem.kv_proj` | `D_n*R_k*s_Wk + D_v*R_k*s_Wv + b_k*(D_n*s_Kn + D_v*s_V + R_k*n_buf*s_Ck) + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch → D_n*R_k*s_Wk + D_v*R_k*s_Wv + b_k*(D_n*s_Kn + D_v*s_V + R_k*n_buf*s_Ck) + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, R_k, b_k, b_q, b_rk, kv_buffers, materialize.expanded_kv, q_resident, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, scratch_bytes, v_load | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=vmem | measured spill; spill probability |
| envelope[vmem:score] | `pressure.envelope.vmem.score` | `b_k*(D_n*s_Kn + D_r*n_buf*s_Kr + D_v*s_V) + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch → b_k*(D_n*s_Kn + D_r*n_buf*s_Kr + D_v*s_V) + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, kv_buffers, s_Kn, s_Kr, s_Qn, s_Qr, s_V, scratch_bytes, v_load | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=vmem | measured spill; spill probability |
| envelope[vmem:softmax] | `pressure.envelope.vmem.softmax` | `D_v*b_k*s_V + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch → D_v*b_k*s_V + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, v_load | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=vmem | measured spill; spill probability |
| envelope[vmem:pv] | `pressure.envelope.vmem.pv` | `D_v*b_k*s_V + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch → D_v*b_k*s_V + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=vmem | measured spill; spill probability |
| envelope[vmem:finalize] | `pressure.envelope.vmem.finalize` | `b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_O + s_LSE) + s_scratch → b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_O + s_LSE) + s_scratch` |  | byte | symbolic | D_n, D_r, D_v, b_q, outputs, q_resident, s_LSE, s_O, s_Qn, s_Qr, scratch_bytes | a*b_q*b_k + b*b_q + c*b_k + d (+ remainder) for this stage, level=vmem | measured spill; spill probability |
...
| excess_over_budget[vreg:kv_proj] | `pressure.capacity_constraint.vreg.kv_proj` | `Max(0, -R_vreg + b_q*(D_v*s_A + 2*s_state)) → Max(0, -R_vreg + b_q*(D_v*s_A + 2*s_state))` |  | byte | symbolic | D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp, vreg_budget_bytes | max(0, M_live - R) at the kv_proj stage, level vreg: data that cannot stay at the level under the source-level model | spill write traffic; compiled live set |
| b_k_max[vreg:score] | `pressure.feasible.vreg.score.b_k` | `floor((R_vreg - b_q*(D_v*s_A + 2*s_state))/(2*b_q*s_X)) → floor((R_vreg - b_q*(D_v*s_A + 2*s_state))/(2*b_q*s_X))` |  | rows | symbolic | D_v, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, s_A, s_X, s_state, score_alias_exp, vreg_budget_bytes | floor((R - b b_q - d)/(a b_q + c)) for the score stage at level vreg under the declared budget R_vreg |  |
| excess_over_budget[vreg:score] | `pressure.capacity_constraint.vreg.score` | `Max(0, -R_vreg + 2*b_k*b_q*s_X + b_q*(D_v*s_A + 2*s_state)) → Max(0, -R_vreg + 2*b_k*b_q*s_X + b_q*(D_v*s_A + 2*s_state))` |  | byte | symbolic | D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, s_A, s_X, s_state, score_alias_exp, vreg_budget_bytes | max(0, M_live - R) at the score stage, level vreg: data that cannot stay at the level under the source-level model | spill write traffic; compiled live set |
| b_k_max[vreg:softmax] | `pressure.feasible.vreg.softmax.b_k` | `floor((R_vreg - b_q*(D_v*s_A + 3*s_state))/(b_q*(s_E + s_X))) → floor((R_vreg - b_q*(D_v*s_A + 3*s_state))/(b_q*(s_E + s_X)))` |  | rows | symbolic | D_v, b_q, exp_alias_p_operand, s_A, s_E, s_X, s_state, score_alias_exp, vreg_budget_bytes | floor((R - b b_q - d)/(a b_q + c)) for the softmax stage at level vreg under the declared budget R_vreg |  |
| excess_over_budget[vreg:softmax] | `pressure.capacity_constraint.vreg.softmax` | `Max(0, -R_vreg + b_k*b_q*(s_E + s_X) + b_q*(D_v*s_A + 3*s_state)) → Max(0, -R_vreg + b_k*b_q*(s_E + s_X) + b_q*(D_v*s_A + 3*s_state))` |  | byte | symbolic | D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_X, s_state, score_alias_exp, vreg_budget_bytes | max(0, M_live - R) at the softmax stage, level vreg: data that cannot stay at the level under the source-level model | spill write traffic; compiled live set |
| b_k_max[vreg:pv] | `pressure.feasible.vreg.pv.b_k` | `floor((R_vreg - b_q*(D_v*s_A + 3*s_state))/(b_q*(s_E + s_P))) → floor((R_vreg - b_q*(D_v*s_A + 3*s_state))/(b_q*(s_E + s_P)))` |  | rows | symbolic | D_v, b_q, exp_alias_p_operand, s_A, s_E, s_P, s_state, score_alias_exp, vreg_budget_bytes | floor((R - b b_q - d)/(a b_q + c)) for the pv stage at level vreg under the declared budget R_vreg |  |
| excess_over_budget[vreg:pv] | `pressure.capacity_constraint.vreg.pv` | `Max(0, -R_vreg + b_k*b_q*(s_E + s_P) + b_q*(D_v*s_A + 3*s_state)) → Max(0, -R_vreg + b_k*b_q*(s_E + s_P) + b_q*(D_v*s_A + 3*s_state))` |  | byte | symbolic | D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_P, s_state, score_alias_exp, vreg_budget_bytes | max(0, M_live - R) at the pv stage, level vreg: data that cannot stay at the level under the source-level model | spill write traffic; compiled live set |
...
| excess_over_budget[vreg:finalize] | `pressure.capacity_constraint.vreg.finalize` | `Max(0, -R_vreg + b_q*(D_v*s_A + 2*s_state)) → Max(0, -R_vreg + b_q*(D_v*s_A + 2*s_state))` |  | byte | symbolic | D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp, vreg_budget_bytes | max(0, M_live - R) at the finalize stage, level vreg: data that cannot stay at the level under the source-level model | spill write traffic; compiled live set |
...
| b_k_max[vmem:kv_proj] | `pressure.feasible.vmem.kv_proj.b_k` | `floor((-D_n*R_k*s_Wk - D_v*R_k*s_Wv + R_vmem - b_q*(D_n*s_Qn + D_r*s_Qr) - s_scratch)/(D_n*s_Kn + D_v*s_V + R_k*n_buf*s_Ck)) → floor((-D_n*R_k*s_Wk - D_v*R_k*s_Wv + R_vmem - b_q*(D_n*s_Qn + D_r*s_Qr) - s_scratch)/(D_n*s_Kn + D_v*s_V + R_k*n_buf*s_Ck))` |  | rows | symbolic | D_n, D_r, D_v, R_k, b_q, b_rk, kv_buffers, materialize.expanded_kv, q_resident, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, scratch_bytes, v_load, vmem_budget_bytes | floor((R - b b_q - d)/(a b_q + c)) for the kv_proj stage at level vmem under the declared budget R_vmem |  |
| excess_over_budget[vmem:kv_proj] | `pressure.capacity_constraint.vmem.kv_proj` | `Max(0, D_n*R_k*s_Wk + D_v*R_k*s_Wv - R_vmem + b_k*(D_n*s_Kn + D_v*s_V + R_k*n_buf*s_Ck) + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch) → Max(0, D_n*R_k*s_Wk + D_v*R_k*s_Wv - R_vmem + b_k*(D_n*s_Kn + D_v*s_V + R_k*n_buf*s_Ck) + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch)` |  | byte | symbolic | D_n, D_r, D_v, R_k, b_k, b_q, b_rk, kv_buffers, materialize.expanded_kv, q_resident, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, scratch_bytes, v_load, vmem_budget_bytes | max(0, M_live - R) at the kv_proj stage, level vmem: data that cannot stay at the level under the source-level model | spill write traffic; compiled live set |
| b_k_max[vmem:score] | `pressure.feasible.vmem.score.b_k` | `floor((R_vmem - b_q*(D_n*s_Qn + D_r*s_Qr) - s_scratch)/(D_n*s_Kn + D_r*n_buf*s_Kr + D_v*s_V)) → floor((R_vmem - b_q*(D_n*s_Qn + D_r*s_Qr) - s_scratch)/(D_n*s_Kn + D_r*n_buf*s_Kr + D_v*s_V))` |  | rows | symbolic | D_n, D_r, D_v, b_q, kv_buffers, s_Kn, s_Kr, s_Qn, s_Qr, s_V, scratch_bytes, v_load, vmem_budget_bytes | floor((R - b b_q - d)/(a b_q + c)) for the score stage at level vmem under the declared budget R_vmem |  |
| excess_over_budget[vmem:score] | `pressure.capacity_constraint.vmem.score` | `Max(0, -R_vmem + b_k*(D_n*s_Kn + D_r*n_buf*s_Kr + D_v*s_V) + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch) → Max(0, -R_vmem + b_k*(D_n*s_Kn + D_r*n_buf*s_Kr + D_v*s_V) + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch)` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, kv_buffers, s_Kn, s_Kr, s_Qn, s_Qr, s_V, scratch_bytes, v_load, vmem_budget_bytes | max(0, M_live - R) at the score stage, level vmem: data that cannot stay at the level under the source-level model | spill write traffic; compiled live set |
| b_k_max[vmem:softmax] | `pressure.feasible.vmem.softmax.b_k` | `floor((R_vmem - b_q*(D_n*s_Qn + D_r*s_Qr) - s_scratch)/(D_v*s_V)) → floor((R_vmem - b_q*(D_n*s_Qn + D_r*s_Qr) - s_scratch)/(D_v*s_V))` |  | rows | symbolic | D_n, D_r, D_v, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, v_load, vmem_budget_bytes | floor((R - b b_q - d)/(a b_q + c)) for the softmax stage at level vmem under the declared budget R_vmem |  |
| excess_over_budget[vmem:softmax] | `pressure.capacity_constraint.vmem.softmax` | `Max(0, D_v*b_k*s_V - R_vmem + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch) → Max(0, D_v*b_k*s_V - R_vmem + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch)` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, v_load, vmem_budget_bytes | max(0, M_live - R) at the softmax stage, level vmem: data that cannot stay at the level under the source-level model | spill write traffic; compiled live set |
| b_k_max[vmem:pv] | `pressure.feasible.vmem.pv.b_k` | `floor((R_vmem - b_q*(D_n*s_Qn + D_r*s_Qr) - s_scratch)/(D_v*s_V)) → floor((R_vmem - b_q*(D_n*s_Qn + D_r*s_Qr) - s_scratch)/(D_v*s_V))` |  | rows | symbolic | D_n, D_r, D_v, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, vmem_budget_bytes | floor((R - b b_q - d)/(a b_q + c)) for the pv stage at level vmem under the declared budget R_vmem |  |
| excess_over_budget[vmem:pv] | `pressure.capacity_constraint.vmem.pv` | `Max(0, D_v*b_k*s_V - R_vmem + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch) → Max(0, D_v*b_k*s_V - R_vmem + b_q*(D_n*s_Qn + D_r*s_Qr) + s_scratch)` |  | byte | symbolic | D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, vmem_budget_bytes | max(0, M_live - R) at the pv stage, level vmem: data that cannot stay at the level under the source-level model | spill write traffic; compiled live set |
...
| excess_over_budget[vmem:finalize] | `pressure.capacity_constraint.vmem.finalize` | `Max(0, -R_vmem + b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_O + s_LSE) + s_scratch) → Max(0, -R_vmem + b_q*(D_n*s_Qn + D_r*s_Qr + D_v*s_O + s_LSE) + s_scratch)` |  | byte | symbolic | D_n, D_r, D_v, b_q, outputs, q_resident, s_LSE, s_O, s_Qn, s_Qr, scratch_bytes, vmem_budget_bytes | max(0, M_live - R) at the finalize stage, level vmem: data that cannot stay at the level under the source-level model | spill write traffic; compiled live set |
| b_k_max[vmem:binding_stage] | `pressure.feasible.vmem.binding` | `min over stages s of floor((R - b_s b_q - d_s)/(a_s b_q + c_s))` |  | rows | unknown | b_q, b_rk, b_rq, materialize.expanded_kv, outputs, q_resident, scratch_bytes, v_load, vmem_budget_bytes | tightest per-stage capacity bound at level vmem | compiled spill/OOM criterion |
...
- `M_live[all:score]`: includes objects whose existence depends on undeclared fields: ['v_load', 'both_qk_branches_live and D_r > 0', 'scratch_bytes']
...
| M_live[all:kv_proj] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, R_k, b_k, b_q, b_rk, exp_alias_p_operand, kv_buffers, materialize.expanded_kv, q_resident, s_A, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, s_state, score_alias_exp, scratch_bytes, v_load | `{}` |  |
| M_live[all:score] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, kv_buffers, s_A, s_Kn, s_Kr, s_Qn, s_Qr, s_V, s_X, s_state, score_alias_exp, scratch_bytes, v_load | `{}` |  |
| M_live[all:softmax] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, exp_alias_p_operand, q_resident, s_A, s_E, s_Qn, s_Qr, s_V, s_X, s_state, score_alias_exp, scratch_bytes, v_load | `{}` |  |
| M_live[all:pv] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, exp_alias_p_operand, q_resident, s_A, s_E, s_P, s_Qn, s_Qr, s_V, s_state, score_alias_exp, scratch_bytes | `{}` |  |
| M_live[all:finalize] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_q, exp_alias_p_operand, outputs, q_resident, s_A, s_LSE, s_O, s_Qn, s_Qr, s_state, score_alias_exp, scratch_bytes | `{}` |  |
| M_peak[all] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, R_k, R_q, b_k, b_q, b_rk, b_rq, both_qk_branches_live and D_r > 0, exp_alias_p_operand, kv_buffers, materialize.expanded_kv, outputs, q_resident, s_A, s_Ck, s_Cq, s_E, s_Kn, s_Kr, s_LSE, s_O, s_P, s_Qacc, s_Qn, s_Qr, s_V, s_Wk, s_Wq, s_Wv, s_X, s_state, score_alias_exp, scratch_bytes, v_load | `{}` |  |
| M_live[vreg:q_proj] | derived | incomplete: depends on undeclared/unbound D_n, b_q, exp_alias_p_operand, s_Qacc, score_alias_exp | `{}` |  |
| M_live[vreg:kv_proj] | derived | incomplete: depends on undeclared/unbound D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp | `{}` |  |
| M_live[vreg:score] | derived | incomplete: depends on undeclared/unbound D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, s_A, s_X, s_state, score_alias_exp | `{}` |  |
| M_live[vreg:softmax] | derived | incomplete: depends on undeclared/unbound D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_X, s_state, score_alias_exp | `{}` |  |
| M_live[vreg:pv] | derived | incomplete: depends on undeclared/unbound D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_P, s_state, score_alias_exp | `{}` |  |
| M_live[vreg:finalize] | derived | incomplete: depends on undeclared/unbound D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp | `{}` |  |
| M_peak[vreg] | derived | incomplete: depends on undeclared/unbound D_n, D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, s_A, s_E, s_P, s_Qacc, s_X, s_state, score_alias_exp | `{}` |  |
| M_live[vmem:q_proj] | derived | incomplete: depends on undeclared/unbound D_n, R_q, b_q, b_rq, s_Cq, s_Wq, scratch_bytes | `{}` |  |
| M_live[vmem:kv_proj] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, R_k, b_k, b_q, b_rk, kv_buffers, materialize.expanded_kv, q_resident, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, scratch_bytes, v_load | `{}` |  |
| M_live[vmem:score] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, kv_buffers, s_Kn, s_Kr, s_Qn, s_Qr, s_V, scratch_bytes, v_load | `{}` |  |
| M_live[vmem:softmax] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, v_load | `{}` |  |
| M_live[vmem:pv] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes | `{}` |  |
| M_live[vmem:finalize] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_q, outputs, q_resident, s_LSE, s_O, s_Qn, s_Qr, scratch_bytes | `{}` |  |
| M_peak[vmem] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, R_k, R_q, b_k, b_q, b_rk, b_rq, kv_buffers, materialize.expanded_kv, outputs, q_resident, s_Ck, s_Cq, s_Kn, s_Kr, s_LSE, s_O, s_Qn, s_Qr, s_V, s_Wk, s_Wq, s_Wv, scratch_bytes, v_load | `{}` |  |
...
| envelope[all:kv_proj] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, R_k, b_k, b_q, b_rk, exp_alias_p_operand, kv_buffers, materialize.expanded_kv, q_resident, s_A, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, s_state, score_alias_exp, scratch_bytes, v_load | `{}` |  |
| envelope[all:score] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, kv_buffers, s_A, s_Kn, s_Kr, s_Qn, s_Qr, s_V, s_X, s_state, score_alias_exp, scratch_bytes, v_load | `{}` |  |
| envelope[all:softmax] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, exp_alias_p_operand, q_resident, s_A, s_E, s_Qn, s_Qr, s_V, s_X, s_state, score_alias_exp, scratch_bytes, v_load | `{}` |  |
| envelope[all:pv] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, exp_alias_p_operand, q_resident, s_A, s_E, s_P, s_Qn, s_Qr, s_V, s_state, score_alias_exp, scratch_bytes | `{}` |  |
| envelope[all:finalize] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_q, exp_alias_p_operand, outputs, q_resident, s_A, s_LSE, s_O, s_Qn, s_Qr, s_state, score_alias_exp, scratch_bytes | `{}` |  |
| envelope[vreg:q_proj] | derived | incomplete: depends on undeclared/unbound D_n, b_q, exp_alias_p_operand, s_Qacc, score_alias_exp | `{}` |  |
| envelope[vreg:kv_proj] | derived | incomplete: depends on undeclared/unbound D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp | `{}` |  |
| envelope[vreg:score] | derived | incomplete: depends on undeclared/unbound D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, s_A, s_X, s_state, score_alias_exp | `{}` |  |
| envelope[vreg:softmax] | derived | incomplete: depends on undeclared/unbound D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_X, s_state, score_alias_exp | `{}` |  |
| envelope[vreg:pv] | derived | incomplete: depends on undeclared/unbound D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_P, s_state, score_alias_exp | `{}` |  |
| envelope[vreg:finalize] | derived | incomplete: depends on undeclared/unbound D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp | `{}` |  |
| envelope[vmem:q_proj] | derived | incomplete: depends on undeclared/unbound D_n, R_q, b_q, b_rq, s_Cq, s_Wq, scratch_bytes | `{}` |  |
| envelope[vmem:kv_proj] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, R_k, b_k, b_q, b_rk, kv_buffers, materialize.expanded_kv, q_resident, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, scratch_bytes, v_load | `{}` |  |
| envelope[vmem:score] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, kv_buffers, s_Kn, s_Kr, s_Qn, s_Qr, s_V, scratch_bytes, v_load | `{}` |  |
| envelope[vmem:softmax] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, v_load | `{}` |  |
| envelope[vmem:pv] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes | `{}` |  |
| envelope[vmem:finalize] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_q, outputs, q_resident, s_LSE, s_O, s_Qn, s_Qr, scratch_bytes | `{}` |  |
...
| excess_over_budget[vreg:kv_proj] | derived | incomplete: depends on undeclared/unbound D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp, vreg_budget_bytes | `{}` |  |
| b_k_max[vreg:score] | derived | incomplete: depends on undeclared/unbound D_v, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, s_A, s_X, s_state, score_alias_exp, vreg_budget_bytes | `{}` |  |
| excess_over_budget[vreg:score] | derived | incomplete: depends on undeclared/unbound D_v, b_k, b_q, both_qk_branches_live and D_r > 0, exp_alias_p_operand, s_A, s_X, s_state, score_alias_exp, vreg_budget_bytes | `{}` |  |
| b_k_max[vreg:softmax] | derived | incomplete: depends on undeclared/unbound D_v, b_q, exp_alias_p_operand, s_A, s_E, s_X, s_state, score_alias_exp, vreg_budget_bytes | `{}` |  |
| excess_over_budget[vreg:softmax] | derived | incomplete: depends on undeclared/unbound D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_X, s_state, score_alias_exp, vreg_budget_bytes | `{}` |  |
| b_k_max[vreg:pv] | derived | incomplete: depends on undeclared/unbound D_v, b_q, exp_alias_p_operand, s_A, s_E, s_P, s_state, score_alias_exp, vreg_budget_bytes | `{}` |  |
| excess_over_budget[vreg:pv] | derived | incomplete: depends on undeclared/unbound D_v, b_k, b_q, exp_alias_p_operand, s_A, s_E, s_P, s_state, score_alias_exp, vreg_budget_bytes | `{}` |  |
| b_k_max[vreg:finalize] | derived | not applicable under the declared configuration | `{}` |  |
| excess_over_budget[vreg:finalize] | derived | incomplete: depends on undeclared/unbound D_v, b_q, exp_alias_p_operand, s_A, s_state, score_alias_exp, vreg_budget_bytes | `{}` |  |
...
| b_k_max[vmem:kv_proj] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, R_k, b_q, b_rk, kv_buffers, materialize.expanded_kv, q_resident, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, scratch_bytes, v_load, vmem_budget_bytes | `{}` |  |
| excess_over_budget[vmem:kv_proj] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, R_k, b_k, b_q, b_rk, kv_buffers, materialize.expanded_kv, q_resident, s_Ck, s_Kn, s_Qn, s_Qr, s_V, s_Wk, s_Wv, scratch_bytes, v_load, vmem_budget_bytes | `{}` |  |
| b_k_max[vmem:score] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_q, kv_buffers, s_Kn, s_Kr, s_Qn, s_Qr, s_V, scratch_bytes, v_load, vmem_budget_bytes | `{}` |  |
| excess_over_budget[vmem:score] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, kv_buffers, s_Kn, s_Kr, s_Qn, s_Qr, s_V, scratch_bytes, v_load, vmem_budget_bytes | `{}` |  |
| b_k_max[vmem:softmax] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, v_load, vmem_budget_bytes | `{}` |  |
| excess_over_budget[vmem:softmax] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, v_load, vmem_budget_bytes | `{}` |  |
| b_k_max[vmem:pv] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, vmem_budget_bytes | `{}` |  |
| excess_over_budget[vmem:pv] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_k, b_q, q_resident, s_Qn, s_Qr, s_V, scratch_bytes, vmem_budget_bytes | `{}` |  |
| b_k_max[vmem:finalize] | derived | not applicable under the declared configuration | `{}` |  |
| excess_over_budget[vmem:finalize] | derived | incomplete: depends on undeclared/unbound D_n, D_r, D_v, b_q, outputs, q_resident, s_LSE, s_O, s_Qn, s_Qr, scratch_bytes, vmem_budget_bytes | `{}` |  |
| b_k_max[vmem:binding_stage] | derived | incomplete: depends on undeclared/unbound b_q, b_rk, b_rq, materialize.expanded_kv, outputs, q_resident, scratch_bytes, v_load, vmem_budget_bytes | `{}` |  |
...
| d(accumulator_bytes)/d(b_q) | `sens.accumulator_bytes.b_q` | `D_v*s_A → D_v*s_A` |  | byte per row | symbolic | D_v, s_A | continuous relaxation of the source-level logical size | a discrete tile change |
...
| d(accumulator_bytes)/d(b_q) | derived | incomplete: depends on undeclared/unbound D_v, s_A | `{}` |  |
...
| metric | formula id | expression | value | unit | status | missing | scope | not equivalent to |
|---|---|---|---|---|---|---|---|---|
| T_LB[mxu] | `perf.lb.mxu` | `(2*B*D_n*H*R_q*S_q + 2*C_pad*D_n + 2*C_pad*D_r + 2*C_pad*D_v + 2*D_n*H*N_keyvisits*R_k + 2*D_v*H*N_keyvisits*R_k)/P_mxu` |  | s | unknown | B, C_pad, D_n, D_r, D_v, H, N_keyvisits, P_mxu, R_k, R_q, S_q, executed_extent_policy, mask, matmul_input_dtype, projection_in_kv_loop, projection_scope, rect_policy, schedule | scope=None | actual latency; calibrated prediction |
| T_LB[vpu] | `perf.lb.vpu` | `(2*B*D_v*H*S_q + 2*B*H*S_q + C_masked + 2*C_pad + 2*D_v*N_rowvisits + 2*N_rowvisits + Piecewise((C_pad, D_r > 0), (0, True)))/P_vpu` |  | s | unknown | B, C_masked, C_pad, D_r, D_v, H, N_rowvisits, P_vpu, S_q, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | scope=None | actual latency; calibrated prediction |
| T_LB[reduce] | `perf.lb.reduce` | `(2*C_pad - N_rowvisits)/P_red` |  | s | unknown | C_pad, N_rowvisits, P_red, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | scope=None | actual latency; calibrated prediction |
| T_LB[layout] | `perf.lb.layout` | `(C_pad + D_n*H*N_keyvisits)/P_layout` |  | s | unknown | C_pad, D_n, H, N_keyvisits, P_layout, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | scope=None | actual latency; calibrated prediction |
| T_LB[exp] | `perf.lb.exp` | `(C_pad + N_rowvisits)/P_exp` |  | s | unknown | C_pad, N_rowvisits, P_exp, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | scope=None | actual latency; calibrated prediction |
| T_LB[path:hbm_to_vmem] | `perf.lb.path:hbm_to_vmem` | `(B*D_n*H*R_k*s_Wk*ceiling(S_q/b_q) + B*D_n*H*R_q*s_Wq*ceiling(S_q/b_q) + B*D_n*H*S_q*s_Qn + B*D_r*H*S_q*s_Qr + B*D_v*H*R_k*s_Wv*ceiling(S_q/b_q) + B*R_q*S_q*s_Cq*ceiling(H/h_pp) + D_r*N_keyvisits*s_Kr*ceiling(H/h_pp) + N_keyvisits*R_k*s_Ck*ceiling(H/h_pp))/W_hbm` |  | s | unknown | B, D_n, D_r, D_v, H, N_keyvisits, R_k, R_q, S_q, W_hbm, b_q, executed_extent_policy, h_pp, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, outputs, projection_scope, rect_policy, s_Ck, s_Cq, s_Kr, s_Qn, s_Qr, s_Wk, s_Wq, s_Wv, schedule | scope=None | actual latency; calibrated prediction |
| T_LB[path:vmem_to_hbm] | `perf.lb.path:vmem_to_hbm` | `(B*D_n*H*S_q*s_Qn + B*D_v*H*S_q*s_O + B*H*S_q*s_LSE)/W_hbm_w` |  | s | unknown | B, D_n, D_v, H, S_q, W_hbm_w, executed_extent_policy, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, outputs, projection_scope, rect_policy, s_LSE, s_O, s_Qn, schedule | scope=None | actual latency; calibrated prediction |
| T_LB[path:vmem_to_vreg] | `perf.lb.path:vmem_to_vreg` | `(B*D_n*H*b_q*s_Qn*ceiling(S_q/b_q) + B*D_r*H*b_q*s_Qr*ceiling(S_q/b_q) + D_n*N_kvblocks*b_k*s_Kn + D_r*N_kvblocks*b_k*s_Kr + D_v*N_kvblocks*b_k*s_V)/W_vmem` |  | s | unknown | B, D_n, D_r, D_v, H, N_kvblocks, S_q, W_vmem, b_k, b_q, executed_extent_policy, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, outputs, projection_scope, q_resident, rect_policy, register_schedule_evidence, s_Kn, s_Kr, s_Qn, s_Qr, s_V, schedule | scope=None | actual latency; calibrated prediction |
| T_LB[critical_path] | `perf.lb.critical_path` | `CP` |  | s | unknown | CP | scope=None | actual latency; calibrated prediction |
| T_LB[combined:scenario=full_overlap_max] | `perf.lb.combined.full_overlap_max` | `max((2*B*D_n*H*R_q*S_q + 2*C_pad*D_n + 2*C_pad*D_r + 2*C_pad*D_v + 2*D_n*H*N_keyvisits*R_k + 2*D_v*H*N_keyvisits*R_k)/P_mxu, (2*B*D_v*H*S_q + 2*B*H*S_q + C_masked + 2*C_pad + 2*D_v*N_rowvisits + 2*N_rowvisits + Piecewise((C_pad, D_r > 0), (0, True)))/P_vpu, (2*C_pad - N_rowvisits)/P_red, (C_pad + D_n*H*N_keyvisits)/P_layout, (C_pad + N_rowvisits)/P_exp, (B*D_n*H*R_k*s_Wk*ceiling(S_q/b_q) + B*D_n*H*R_q*s_Wq*ceiling(S_q/b_q) + B*D_n*H*S_q*s_Qn + B*D_r*H*S_q*s_Qr + B*D_v*H*R_k*s_Wv*ceiling(S_q/b_q) + B*R_q*S_q*s_Cq*ceiling(H/h_pp) + D_r*N_keyvisits*s_Kr*ceiling(H/h_pp) + N_keyvisits*R_k*s_Ck*ceiling(H/h_pp))/W_hbm, (B*D_n*H*S_q*s_Qn + B*D_v*H*S_q*s_O + B*H*S_q*s_LSE)/W_hbm_w, (B*D_n*H*b_q*s_Qn*ceiling(S_q/b_q) + B*D_r*H*b_q*s_Qr*ceiling(S_q/b_q) + D_n*N_kvblocks*b_k*s_Kn + D_r*N_kvblocks*b_k*s_Kr + D_v*N_kvblocks*b_k*s_V)/W_vmem, CP)` |  | s | unknown | B, CP, C_masked, C_pad, D_n, D_r, D_v, H, N_keyvisits, N_kvblocks, N_rowvisits, P_exp, P_layout, P_mxu, P_red, P_vpu, R_k, R_q, S_q, W_hbm, W_hbm_w, W_vmem, b_k, b_q, executed_extent_policy, exp_base, h_pp, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, matmul_input_dtype, outputs, overlap_model, projection_in_kv_loop, projection_scope, q_resident, rect_policy, recurrence, register_schedule_evidence, s_Ck, s_Cq, s_Kn, s_Kr, s_LSE, s_O, s_Qn, s_Qr, s_V, s_Wk, s_Wq, s_Wv, schedule | overlap scenario full_overlap_max | calibrated prediction; measured time |
| T_LB[combined:scenario=serial_stage_sum] | `perf.lb.combined.serial_stage_sum` | `unknown` |  | s | unknown | B, CP, C_masked, C_pad, D_n, D_r, D_v, H, N_keyvisits, N_kvblocks, N_rowvisits, P_exp, P_layout, P_mxu, P_red, P_vpu, R_k, R_q, S_q, W_hbm, W_hbm_w, W_vmem, b_k, b_q, executed_extent_policy, exp_base, h_pp, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, matmul_input_dtype, outputs, overlap_model, projection_in_kv_loop, projection_scope, q_resident, rect_policy, recurrence, register_schedule_evidence, s_Ck, s_Cq, s_Kn, s_Kr, s_LSE, s_O, s_Qn, s_Qr, s_V, s_Wk, s_Wq, s_Wv, schedule | overlap scenario serial_stage_sum | calibrated prediction; measured time |
...
| metric | source | coverage | bindings | evidence |
|---|---|---|---|---|
| T_LB[mxu] | derived | incomplete: depends on undeclared/unbound B, C_pad, D_n, D_r, D_v, H, N_keyvisits, P_mxu, R_k, R_q, S_q, executed_extent_policy, mask, matmul_input_dtype, projection_in_kv_loop, projection_scope, rect_policy, schedule | `{}` |  |
| T_LB[vpu] | derived | incomplete: depends on undeclared/unbound B, C_masked, C_pad, D_r, D_v, H, N_rowvisits, P_vpu, S_q, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| T_LB[reduce] | derived | incomplete: depends on undeclared/unbound C_pad, N_rowvisits, P_red, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| T_LB[layout] | derived | incomplete: depends on undeclared/unbound C_pad, D_n, H, N_keyvisits, P_layout, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| T_LB[exp] | derived | incomplete: depends on undeclared/unbound C_pad, N_rowvisits, P_exp, executed_extent_policy, exp_base, mask, outputs, projection_in_kv_loop, projection_scope, rect_policy, recurrence, schedule | `{}` |  |
| T_LB[path:hbm_to_vmem] | derived | incomplete: depends on undeclared/unbound B, D_n, D_r, D_v, H, N_keyvisits, R_k, R_q, S_q, W_hbm, b_q, executed_extent_policy, h_pp, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, outputs, projection_scope, rect_policy, s_Ck, s_Cq, s_Kr, s_Qn, s_Qr, s_Wk, s_Wq, s_Wv, schedule | `{}` |  |
| T_LB[path:vmem_to_hbm] | derived | incomplete: depends on undeclared/unbound B, D_n, D_v, H, S_q, W_hbm_w, executed_extent_policy, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, outputs, projection_scope, rect_policy, s_LSE, s_O, s_Qn, schedule | `{}` |  |
| T_LB[path:vmem_to_vreg] | derived | incomplete: depends on undeclared/unbound B, D_n, D_r, D_v, H, N_kvblocks, S_q, W_vmem, b_k, b_q, executed_extent_policy, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, outputs, projection_scope, q_resident, rect_policy, register_schedule_evidence, s_Kn, s_Kr, s_Qn, s_Qr, s_V, schedule | `{}` |  |
| T_LB[critical_path] | derived | incomplete: depends on undeclared/unbound CP | `{}` |  |
| T_LB[combined:scenario=full_overlap_max] | derived | incomplete: depends on undeclared/unbound B, CP, C_masked, C_pad, D_n, D_r, D_v, H, N_keyvisits, N_kvblocks, N_rowvisits, P_exp, P_layout, P_mxu, P_red, P_vpu, R_k, R_q, S_q, W_hbm, W_hbm_w, W_vmem, b_k, b_q, executed_extent_policy, exp_base, h_pp, heads_per_program, mask, materialize.expanded_kv, materialize.q_nope, matmul_input_dtype, outputs, overlap_model, projection_in_kv_loop, projection_scope, q_resident, rect_policy, recurrence, register_schedule_evidence, s_Ck, s_Cq, s_Kn, s_Kr, s_LSE, s_O, s_Qn, s_Qr, s_V, s_Wk, s_Wq, s_Wv, schedule | `{}` |  |
...
```json
{
  "bounds": [
    ...
    {
      "resource": "vpu",
      "numerator": "2*B*D_v*H*S_q + 2*B*H*S_q + C_masked + 2*C_pad + 2*D_v*N_rowvisits + 2*N_rowvisits + Piecewise((C_pad, D_r > 0), (0, True))",
      "denominator": "P_vpu",
      "expression": "(2*B*D_v*H*S_q + 2*B*H*S_q + C_masked + 2*C_pad + 2*D_v*N_rowvisits + 2*N_rowvisits + Piecewise((C_pad, D_r > 0), (0, True)))/P_vpu",
      "value": null,
      "unit": "s",
      "scope": null,
      "status": "unknown",
    ...
    },
    {
      ...
      "numerator": "B*D_n*H*S_q*s_Qn + B*D_v*H*S_q*s_O + B*H*S_q*s_LSE",
      "denominator": "W_hbm_w",
      "expression": "(B*D_n*H*S_q*s_Qn + B*D_v*H*S_q*s_O + B*H*S_q*s_LSE)/W_hbm_w",
      "value": null,
      "unit": "s",
      "scope": null,
      "status": "unknown",
